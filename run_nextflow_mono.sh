...
input=`realpath -e $1`
output=`realpath $2`
vcf_infile=`realpath $3`
bam_infile=`realpath $4`
email=$5
...
#SBATCH --job-name Nextflow_RNA_mono
...
/hpc/diaggen/software/tools/nextflow run $workflow_path/RNA_mono.nf \
-c $workflow_path/RNA_mono.config \
--fastq_path $input \
--outdir $output \
--vcf_infile $vcf_infile \
--bam_infile $bam_infile \
--email $email \
-profile slurm \
...
if [ \$? -eq 0 ]; then
    ...
    echo "RNA mono workflow completed successfully."
    rm workflow.running
    touch workflow.done
